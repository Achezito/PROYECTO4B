...
export default function App() {
  ...
  const handleAddComponent = () => {
    ...
    axios.post(url, newComponent)
      ...
      .catch((error) => {
        alert(error.response?.data?.error || "No se pudo agregar el componente");
      });
  };
    ...
    <View style={styles.container}>
      ...
      {Object.keys(formData).map((key) => (
        <TextInput
          ...
          keyboardType={["RAM_slots", "max_RAM", "expansion_slots", "capacity", "voltage", "quantity", "audio_channels", "id_type"].includes(key) ? 'numeric' : 'default'}
        />
      ))}

      <View style={styles.buttonContainer}>
        <Button title='Agregar Componente' onPress={handleAddComponent} color="#007bff" />
      </View>
