...
export default function SuministrosScreen({ route, navigation }) {
  ...
  const fetchSuministros = async () => {
    try {
      ...
      Alert.alert('Error', 'No se pudieron cargar los suministros. Intenta nuevamente.');
    } finally {
      setLoading(false);
    }
  };
  ...
  const renderItem = ({ item }) => (
    <TouchableOpacity style={styles.card}>
      <Text style={styles.cardTitle}>{item.material_model || 'Material sin nombre'}</Text>
      <Text style={styles.cardSubtitle}>{item.material_type || 'Tipo de material no especificado'}</Text>
      <Text style={styles.cardSubtitle}>Pertenece a la orden: {item.order_id || 'Orden no especificada'}</Text>
      
      <View style={styles.detailContainer}>
        <Text style={styles.cardText}>Cantidad: {item.quantity || 'No especificada'}</Text>
        <Text style={styles.cardText}>Proveedor: {item.supplier_name || 'Sin proveedor'}</Text>
        <Text style={styles.cardText}>Contacto: {item.supplier_contact || 'Sin contacto'}</Text>
        <Text style={styles.cardText}>Dirección: {item.supplier_address || 'Sin dirección'}</Text>
        <Text style={styles.cardText}>Estado del Suministro: {item.supply_status || 'No especificado'}</Text>
      </View>
    </TouchableOpacity>
  );
  ...
  return (
    <View style={styles.container}>
      <Text style={styles.title}>
        {id_sub_warehouse ? `Suministros del Subalmacén ${id_sub_warehouse}` : 'Suministros'}
      </Text>
      ...
      {loading ? (
        ...
        <FlatList
          ...
          ListEmptyComponent={<Text style={styles.emptyText}>No hay suministros disponibles.</Text>}
        />
      )}
    </View>
  );
