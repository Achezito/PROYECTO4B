...
export default function UpdateSubWarehouseScreen({ route, navigation }) {
  ...
  const fetchSubWarehouses = async () => {
    try {
      const response = await fetch(`http://localhost/PROYECTO4B-1/phpfiles/react/sub_warehouse_api.php?id=${warehouseId}`);
  ...
  const handleUpdateSubWarehouse = async () => {
    if (!selectedSubWarehouse) {
      Alert.alert('Error', 'Por favor, selecciona un subalmacén para actualizar.');
      return;
    }
    ...
    try {
      ...
      if (response.ok) {
        Alert.alert('Éxito', `Subalmacén "${location}" actualizado correctamente.`);
        setLocation('');
        setCapacity('');
        setSelectedCategory('');
        ...
        Alert.alert('Error', result.message || 'No se pudo actualizar el subalmacén.');
      }
    } catch (error) {
  ...
  return (
    <View style={styles.container}>
      ...
      <FlatList
        ...
        ListEmptyComponent={<Text style={styles.emptyText}>No hay subalmacenes disponibles</Text>}
      />
      ...
      {selectedSubWarehouse && (
        <>
          ...
          <TouchableOpacity style={styles.button} onPress={handleUpdateSubWarehouse} disabled={loading}>
            <Text style={styles.buttonText}>{loading ? 'Actualizando...' : 'Actualizar Subalmacén'}</Text>
          </TouchableOpacity>
        </>
      )}
    </View>
