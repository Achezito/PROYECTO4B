...
export default function NuevaTransaccionScreen({ route, navigation }) {
  ...
  const handleMaterialChange = (materialId) => {
    const material = materials.find((m) => m.id_material === parseInt(materialId));
  ...
  const handleTransaction = async () => {
    ...
    if (type === 'outbound' && parseInt(quantity) > availableQuantity) {
      Alert.alert('Error', 'La cantidad no puede exceder la cantidad disponible.');
      return;
    }
    ...
    try {
      ...
      if (response.ok) {
        ...
        Alert.alert('Error', data.error || 'Hubo un problema al registrar la transacción.');
      }
    } catch (error) {
          ...
          {materials.map((material) => (
            <Picker.Item
              ...
              label={`${material.Material} ${
                type === 'outbound' ? `(Disponible: ${material['Cantidad Disponible']})` : ''
              }`}
